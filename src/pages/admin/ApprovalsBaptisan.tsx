...
import { CheckCircle, XCircle, ChevronDown, ChevronUp, Trash2, Droplets } from 'lucide-react';
import { RegistrationItem } from '../../types';
import clsx from 'clsx';

export default function ApprovalsBaptisan() {
  ...
  const filteredData = data.filter(item => item.type === 'baptisan');
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Yakin ingin menghapus data pendaftaran baptisan ini?')) return;
    try {
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-navy flex items-center gap-2">
          <Droplets className="text-blue-600" size={24} /> Pendaftaran Baptisan
        </h1>
      </div>
          ...
          <table className="w-full text-left text-sm text-navy">
            ...
            <tbody className="divide-y divide-border-subtle">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="text-center py-12 text-text-muted">Tidak ada pendaftaran baptisan.</td>
                </tr>
              ) : (
                filteredData.map(item => (
                  <React.Fragment key={item.id}>
                    <tr className="hover:bg-sand-darker/50 transition-colors cursor-pointer" onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}>
                      ...
                      <td className="px-6 py-4 font-medium">{item.namaPendaftar || (item as any).nama_pendaftar || (item as any).nama || 'Pendaftar Baptisan'}</td>
                      <td className="px-6 py-4">{item.nik || (item as any).nik || '-'}</td>
                        ...
                        <td colSpan={6} className="px-14 py-6">
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-sm">
                            <div>
                              ...
                              <p className="font-medium text-navy">{item.namaPendaftar || (item as any).nama_pendaftar || '-'}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className="font-medium text-navy">{item.gender || (item as any).jenis_kelamin || '-'}</p>
                            </div>
                            <div>
                              <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-1">Tanggal Lahir</p>
                              <p className="font-medium text-navy">{item.tanggalLahir || (item as any).tanggal_lahir || '-'}</p>
                            </div>
                            <div>
                              <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-1">No WhatsApp</p>
                              <p className="font-medium text-navy">{item.noHp || (item as any).no_hp || '-'}</p>
                            </div>
                            <div>
                              <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-1">Alamat</p>
                            ...
                            <div>
                              ...
                              <p className="font-medium text-navy">{item.tanggalDaftar ? new Date(item.tanggalDaftar).toLocaleDateString('id-ID') : '-'}</p>
                            </div>
                          </div>
                        </td>
                      </tr>
