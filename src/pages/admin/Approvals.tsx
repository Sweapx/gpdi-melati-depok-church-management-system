                ...
                data.map(item => (
                  <React.Fragment key={item.id}>
                    <tr className="hover:bg-sand-darker/50 transition-colors cursor-pointer" onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}>
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-navy/10 text-navy">
                          {item.type}
                        </span>
                      </td>
                        ...
                        <td colSpan={6} className="px-14 py-6">
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-sm">
                            ...
                            {item.customResponses && Object.keys(item.customResponses).length > 0 && (
                              <div className="col-span-2 md:col-span-3 mt-4 pt-4 border-t border-border-subtle">
                                <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-3">Respon Form Kustom</p>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  {Object.entries(item.customResponses).map(([key, val]) => (
                                    <div key={key} className="bg-white p-3 rounded-lg border border-border-subtle">
                                      <p className="text-[10px] font-bold text-text-muted uppercase tracking-wider mb-1">{key}</p>
